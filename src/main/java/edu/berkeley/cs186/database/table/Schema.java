    ...
    public Record verify(Record record) {
        ...
        for (int i = 0; i < values.size(); ++i) {
            ...
            if (!actual.equals(expected)) {
                ...
                if(actual.getTypeId() == TypeId.LONG && expected.getTypeId() == TypeId.DATE) {
                    // Explicit cast with specific date constructors
                    DataBox dateBox = values.get(i);
                    values.set(i, new DateDataBox(dateBox.getLong()));
                    continue;
                }
                if(actual.getTypeId() == TypeId.STRING && expected.getTypeId() == TypeId.DATE) {
                    // Explicit cast with specific date constructors
                    DataBox dateBox = values.get(i);
                    values.set(i, new DateDataBox(dateBox.getString()));
                    continue;
                }
